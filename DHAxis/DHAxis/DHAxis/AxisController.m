...
@property (nonatomic, strong) AxisRule *rule;
@property (nonatomic, strong) AxisDigitalDivision *digital;
...
- (void)updateAppearance {
    ...
    for (id<Axis> axis in temp) {
        if ([axis isKindOfClass:[AxisRule class]]) {
            _rule = axis;
        } else if ([axis isKindOfClass:[AxisDigitalDivision class]]) {
            _digital = axis;
        }
    }
...
- (void)judgeExistDataInTheInterval:(NSTimeInterval)targetTimeInterval fromDataArray:(NSArray <AxisData *> *)dataArray withPanState:(BOOL)isPaning {
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        ...
        for (int i = 0; i < [dataArray count]; i++) {
            AxisData *subData = [dataArray objectAtIndex:i];
            
            if (subData.startTimeInterval >= targetTimeInterval && subData.endTimeInterval <= targetTimeInterval) {
                if (weakself.delegate && [weakself.delegate respondsToSelector:@selector(axis:didEndedAtDataSection:)]) {
                    [weakself.delegate axis:weakself didEndedAtDataSection:[subData copy]];
                    break;
                }
            }
        }
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSKeyValueChangeKey,id> *)change context:(void *)context {
    ...
    if ([keyPath isEqualToString:@"currentTimeInterval"]) {
        if (_delegate && [_delegate respondsToSelector:@selector(axis:didChangedTimeInterval:)]) {
            [_delegate axis:self didChangedTimeInterval:_currentTimeInterval];
        }
        // 判断停止时是否存在数据
        ...
        if (_delegate && [_delegate respondsToSelector:@selector(axis:didChangedScale:)]) {
            [_delegate axis:self didChangedScale:_currentScale];
        }
    } else if ([keyPath isEqualToString:@"paning"]) {
        
        if (self.isPaning) {
            if (_delegate && [_delegate respondsToSelector:@selector(axisDidBeginScrolling:)]) {
                [_delegate axisDidBeginScrolling:self];
            }
        } else {
            if (_delegate && [_delegate respondsToSelector:@selector(axisDidEndScrolling:)]) {
                [_delegate axisDidEndScrolling:self];
            }
        }
    } else if ([keyPath isEqualToString:@"pinching"]) {
        
        if (self.isPinching) {
            if (_delegate && [_delegate respondsToSelector:@selector(axisDidBeginPinching:)]) {
                [_delegate axisDidBeginPinching:self];
            }
        } else {
            if (_delegate && [_delegate respondsToSelector:@selector(axisDidEndPinching:)]) {
                [_delegate axisDidEndPinching:self];
            }
        }
    }
}
...
- (AxisView *)axisView {
    if (!_axisView) {
        _axisView = [[AxisView alloc] initWithFrame:self.bounds];
    }
    return _axisView;
}
